...
|---|---|---|
| `RigidbodyMovementSystem` (бег/ускорение) | не мигрировано (Update, пишет velocity) | **ВЫСОКИЙ** |
| `SurfaceCheckSystem` (граунд/склон/coyote) | не мигрировано (Update, `Cast` + coyote-таймер) | **ВЫСОКИЙ** |
| `JumpSystem` / `AirJumpSystem` / `WallJumpSystem` | не мигрировано (буфер/заряд на dt, импульс) | **ВЫСОКИЙ** |
