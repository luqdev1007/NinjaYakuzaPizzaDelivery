...
namespace Assets._Project.Develop.Runtime.Configs.Gameplay.Entities
{
    ...
    public class GlideSettings
    {
        ...
        [field: SerializeField, Tooltip("Плавность замедления вертикальной скорости после рывка (одергивания).")]
        public float SpeedDamping { get; private set; } = 5f;

        [field: SerializeField, Tooltip("Сила подскока вверх при закрытии парашюта.")]
        public float BounceForce { get; private set; } = 4f;

        [field: SerializeField, Tooltip("Минимальная скорость падения, при которой можно активировать глайд.")]
        public float MinFallVelocity { get; private set; } = -3f;

        [field: SerializeField, Tooltip("Сила резкого торможения (Snap) в момент раскрытия.")]
        public float SnapSpeed { get; private set; } = 40f;

        [field: SerializeField, Tooltip("Длительность фазы Snap (одергивания).")]
        public float SnapDuration { get; private set; } = 0.15f;

        [field: SerializeField, Tooltip("Множитель противодействия текущей скорости падения при раскрытии (насколько резко гасим инерцию).")]
